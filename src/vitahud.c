...
typedef struct { int year; int month; int day; int hour; int minute; int second; } SceDateTime;
typedef struct { unsigned int buttons; } SceCtrlData;
typedef struct { void *base; int pitch; int pixelformat; int width; int height; } SceDisplayFrameBuf;
typedef struct { unsigned int size; unsigned int size_user; unsigned int size_cdram; unsigned int size_phycont; } SceKernelFreeMemorySizeInfo;
typedef void* tai_hook_ref_t; struct _tai_hook_user { struct _tai_hook_user *next; void *func; void *old; };
#define SCE_CTRL_LTRIGGER 1
#define SCE_CTRL_RTRIGGER 2
#define SCE_CTRL_START 4
#define SCE_CTRL_SELECT 8
#define SCE_CTRL_UP 16
#define SCE_CTRL_DOWN 32
#define SCE_CTRL_LEFT 64
#define SCE_CTRL_RIGHT 128
#define SCE_CTRL_CROSS 256
#define SCE_CTRL_CIRCLE 512
#define SCE_CTRL_TRIANGLE 1024
#define SCE_CTRL_SQUARE 2048
...
#define TAI_MAIN_MODULE 0
int scePowerGetBatteryLifePercent(void); int scePowerIsBatteryCharging(void); int scePowerGetBatteryTemp(void);
int scePowerGetArmClockFrequency(void); int scePowerGetBusClockFrequency(void); int scePowerGetGpuClockFrequency(void); int scePowerGetGpuXbarClockFrequency(void);
int sceIoMkdir(const char*, int); int sceIoRemove(const char*); SceUID sceIoOpen(const char*, int, int); int sceIoRead(SceUID, void*, int); int sceIoWrite(SceUID, const void*, int); int sceIoClose(SceUID);
int sceRtcGetCurrentTick(SceRtcTick*); int sceRtcGetCurrentClockLocalTime(SceDateTime*); int sceCtrlPeekBufferPositive(int, SceCtrlData*, int); int sceDisplaySetFrameBuf(const SceDisplayFrameBuf*, int); int sceDisplayGetFrameBuf(SceDisplayFrameBuf*, int);
int sceKernelCreateThread(const char*, int (*)(SceSize, void*), int, int, int, int, void*); int sceKernelStartThread(SceUID, int, void*); int sceKernelDelayThread(int); int sceKernelExitDeleteThread(int); int sceKernelDeleteThread(SceUID);
int sceKernelGetFreeMemorySize(SceKernelFreeMemorySizeInfo*); int sceKernelGetModel(void); int sceKernelGetProcessId(void); int sceAppMgrAppParamGetString(int, int, char*, int); int sceAppMgrGetNameById(int, char*);
int taiHookFunctionImport(tai_hook_ref_t*, const char*, int, int, const void*); int taiHookRelease(SceUID, tai_hook_ref_t);
